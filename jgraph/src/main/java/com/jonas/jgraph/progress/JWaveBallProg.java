...
//            mAmplitude = mAmplitudeTemp = typedArray.getFloat(R.styleable.JProgBall_amplitude, 10);
    ...
    protected void onFinishInflate() {
        ...
        mBackWavePaint.setColor(ballgroundColor);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mProgRectf = new RectF(mCenter.x - mWaveBallRadius, mCenter.y - mWaveBallRadius, mCenter.x + mWaveBallRadius, mCenter.y + mWaveBallRadius);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (tier > 0) {
            ...
            canvas.drawCircle(mCenter.x, mCenter.y, mWaveBallRadius, mBackWavePaint);
        }

        if (mProgMode == ProgMode.PROG_WAVE) {
//            drawWaveProgressByOP(canvas, (float) (getWaveLength()/3));
//            mWavePaint.setColor(Color.GREEN);
            drawWaveProgressByOP(canvas, 0);
        } else {
            //画进度
            drawProgress(canvas);
        }
    ...
    private void drawWaveProgressByOP(Canvas canvas, float offset) {
...
//            mWavePath.lineTo(i, currentY + mAmplitude * (float) Math.cos((i + waveMove) * (waveNum * 180f / (2 * mWaveBallRadius)) * Math.PI / 180f));
        ...
        if (showProgressLine) {
            ...
            canvas.drawLine(0, currentY, mWidth, currentY, mBackWavePaint);
        }
        if (!stopWaving) {
...
//        double acos = Math.acos((mWaveBallRadius - waveProg) / mWaveBallRadius) / Math.PI * 180;
     ...
     * 需要的话 分配好各层的颜色
     *
     * @param progress 设置进度 并会根据需要配置随机颜色
     */
